...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        positions = torch.arange(0, x.size(1), device=x.device).repeat(x.size(0), 1)
...
class TransformerSlidingAttentionPositionalModel(nn.Module):
    def __init__(self, accel_input_size, positional_embedding_size, accel_embedding_size, num_heads, num_transformer_blocks, window_size, num_classes):
        super(TransformerSlidingAttentionPositionalModel, self).__init__()
        self.name = 'TransformerSlidingAttentionPositionalv1'
        self.num_classes = num_classes

        self.positional_embedding = PositionalEmbedding(positional_embedding_size)
        self.accel_embedding = AccelerometerEmbedding(accel_input_size, accel_embedding_size)
        
        # Concatenate both embeddings
        hidden_size = positional_embedding_size + accel_embedding_size
    ...
    def forward(self, accel_data, mask = None):
        positional_embedding = self.positional_embedding(accel_data)
        accel_embedding = self.accel_embedding(accel_data)
        
        combined_embedding = torch.cat([positional_embedding, accel_embedding], dim=-1)
...
def train_model(model, data_loader, criterion, optimizer):
    ...
    for batch in tqdm(data_loader, leave=False):
        ...
        # Forward pass
        logits = model(accelerometer_data)
...
sequence_length = 250  # Adjust based on your needs   #this should be much much larger
...
model = TransformerSlidingAttentionPositionalModel(accel_input_size, positional_embedding_size, accel_embedding_size, num_heads, num_transformer_blocks, window_size, num_classes).to(device)
